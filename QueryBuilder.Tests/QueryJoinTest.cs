...
public class QueryJoinTest
{
    ...
    public void BasicJoin()
    {
        ...
        Assert.Equal("SELECT * FROM `users` INNER JOIN `countries` ON `countries`.`id` = `users`.`country_id`", c[1]);
    }
    ...
    public void JoinTypes(string given, string output)
    {
        ...
        Assert.Equal($"SELECT * FROM [users] {output} [countries] ON [countries].[id] = [users].[country_id]", c[0]);

        Assert.Equal($"SELECT * FROM `users` {output} `countries` ON `countries`.`id` = `users`.`country_id`", c[1]);

        Assert.Equal($"SELECT * FROM \"users\" {output} \"countries\" ON \"countries\".\"id\" = \"users\".\"country_id\"", c[2]);
