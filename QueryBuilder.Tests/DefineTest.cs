...
public class DefineTest : TestSupport
{
    ...
    public void Test_Define_WhereEnds()
    {
        ...
        Assert.Equal("SELECT [ProductId] FROM [Products] WHERE LOWER([ProductName]) like '%coffee'",
            c1[EngineCodes.SqlServer]);

        Assert.Equal("SELECT [ProductId], [ProductName] FROM [Products] WHERE [ProductName] like '%Coffee'",
            c2[EngineCodes.SqlServer]);
    }
    ...
    public void Test_Define_WhereStarts()
    {
        ...
        Assert.Equal("SELECT [ProductId], [QuantityPerUnit] FROM [Products] WHERE LOWER([QuantityPerUnit]) like '12%'",
            c1[EngineCodes.SqlServer]);
        Assert.Equal("SELECT [ProductId], [QuantityPerUnit] FROM [Products] WHERE [QuantityPerUnit] like '12%'",
            c2[EngineCodes.SqlServer]);
    }
    ...
    public void Test_Define_WhereContains()
    {
        ...
        Assert.Equal(
            ...
            c1[EngineCodes.SqlServer]);
        Assert.Equal("SELECT [ProductId], [QuantityPerUnit] FROM [Products] WHERE [QuantityPerUnit] like '%500%'",
            c2[EngineCodes.SqlServer]);
    }
    ...
    public void Test_Define_WhereLike()
    {
        ...
        Assert.Equal("SELECT [ProductId], [ProductName], [SupplierID] FROM [Products] WHERE [SupplierID] like '20'",
            c2[EngineCodes.SqlServer]);
    }
    ...
    */
    ...
    {
        var dateObj = new DateTime(1996, 8, 1);
        ...
        Assert.Equal("SELECT * FROM [Orders] WHERE CAST([RequiredDate] AS DATE) = '1996-08-01'",
            c[EngineCodes.SqlServer]);
        ...
        Assert.Equal("SELECT * FROM [Orders] WHERE CAST([RequiredDate] AS TIME) != '00:00:00'",
            c3[EngineCodes.SqlServer]);
    }
    ...
    public void Test_Define_WhereExists()
    {
        ...
        var c = Compile(query);
        Assert.Equal("SELECT * FROM [Customers] WHERE EXISTS (SELECT 1 FROM [Orders] WHERE [ShipPostalCode] = '8200')",
            c[EngineCodes.SqlServer]);
    }
